...
- **AI Moderation:** It uses large language models (Anthropic Claude via OpenRouter or Anthropic SDK) to analyze messages and decide if they violate server rules.
...
- `@anthropic-ai/sdk` for AI moderation
- `eslint` and `prettier` for code quality

## Scripts

- `start-dev` and `start-prod` for running the bot with different environments
- `lint`, `prettier`, and `prettier:check` for code formatting and linting

## Configuration

Sensitive data and server-specific settings are loaded from environment variables and config files (not in the repo).
